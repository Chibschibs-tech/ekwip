...
export default function RentalProductCard({ product, brandName }: RentalProductCardProps) {
  const inStock = (product.stockQuantity || 0) > 0
  const minPrice = product.rentalDurations?.[0]?.monthlyFee || product.price
  const defaultDuration = product.rentalDurations?.[0]?.duration || 12
      ...
      <Card className="h-full overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border-0 shadow-lg">
        ...
        <CardContent className="p-6">
          <div className="space-y-3">
            {/* Brand */}
            {brandName && <div className="text-sm text-gray-500 font-medium">{brandName}</div>}
            ...
            <div className="flex items-center justify-between pt-2">
              <div>
                ...
                <div className="text-xl font-bold text-ekwip">
                  {formatPrice(minPrice)} <span className="text-sm font-normal">/mois</span>
                </div>
              </div>
